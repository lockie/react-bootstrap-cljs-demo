...
* [`src/react_bootstrap_cljs_demo/`](src/react_bootstrap_cljs_demo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/react_bootstrap_cljs_demo/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `react-bootstrap-cljs-demo` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/react_bootstrap_cljs_demo/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns react-bootstrap-cljs-demo.example
  (:require [react-bootstrap-cljs-demo.config :as config])
